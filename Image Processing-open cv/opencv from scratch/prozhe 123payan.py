...
def GaussianMask(sizex, sizey, sigma=33, center=None, fix=1):
    ...
    """
    x = np.arange(0, sizex, 1, float)#start stop step dtype
...
if __name__ == '__main__':
    ...
    fix_arr = np.random.randn(num_subjects,10000)
    fix_arr -= fix_arr.min()
    fix_arr /= fix_arr.max()
    fix_arr[:, 0] *= W
    fix_arr[:, 1] *= H
    ...
    # Create heatmap
    heatmap = Fixpos2Densemap(fix_arr, W, H, img, 0.1,10)
    cv2.imwrite("output.jpg", heatmap)
